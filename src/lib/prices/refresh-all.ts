...
export async function fetchAllPriceResults(holdings: Holding[]): Promise<PriceResult[]> {
  ...
  const cryptoIds = unique(
    holdings
      ...
      .map((holding) => (holding.ticker.toUpperCase() === "BTC" ? "bitcoin" : ""))
  );

  return Promise.all([
    createPriceTask("currency", () => fetchExchangeRates()),
    createPriceTask("indian-mf", () =>
